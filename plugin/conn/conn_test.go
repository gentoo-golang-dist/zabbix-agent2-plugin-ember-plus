...
//nolint:maintidx,paralleltest
func TestConnCollection_reader(t *testing.T) {
	var (
		bTrue  = true
		bFalse = false
	)
				...
				[][]byte{
					{
						...
						0x7f, 0x00, 0x00, 0x01, 0x7f, 0x00, 0x00, 0x01, 0x27, 0x0e, 0xd8, 0x4f, 0x5f, 0x85, 0xe1, 0x0e,
						0x46, 0x58, 0x3e, 0xfd, 0x50, 0x18, 0x27, 0xf9, 0x11, 0xbf, 0x00, 0x00, 0xfe, 0x00, 0x0e, 0x00,
						0x01, 0xc0, 0x01, 0x02, 0x1e, 0x02, 0x60, 0x35, 0x6b, 0x32, 0xa0, 0x30, 0x63, 0x2e, 0xa0, 0x03,
						0x02, 0x01, 0x01, 0xa1, 0x27, 0x31, 0x25, 0xa0, 0x16, 0x0c, 0x14, 0x52, 0x33, 0x4c, 0x41, 0x59,
						0x56, 0x69, 0x72, 0x74, 0x75, 0x61, 0x6c, 0x50, 0x61, 0x74, 0x63, 0x68, 0x42, 0x61, 0x79, 0xa1,
						0x02, 0x0c, 0x00, 0xa4, 0x02, 0x0c, 0x00, 0xa3, 0x03, 0x01, 0x01, 0xfd, 0xdf, 0xb5, 0x11, 0xff,
					},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			go func() {
				for _, p := range tt.args.expectedPath {
					ch.expectedPath <- p
				}
			}()
			...
			if tt.args.expect != nil && *tt.args.expect {
				for i, col := range tt.want {
					got := <-ch.response
					if diff := cmp.Diff(col, got); diff != "" {
						t.Fatalf("TestConnCollection.reader() collection %d diff= %s", i, diff)
					}
				}
			}
