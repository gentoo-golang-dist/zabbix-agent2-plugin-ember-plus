...
func TestDefaultASN1Codec_DecodeUniversal(t *testing.T) {
	tests := []struct {
		...
		{
			"+valid",
			NewDecoder([]byte{UniversalObjectTag, 0x02, 0x01, 0x00}),
			[]int{1, 0},
			false,
		},
		{
		...
		{
			"-invalidByteCount",
			NewDecoder([]byte{UniversalObjectTag}),
			nil,
			true,
		},
		{
			"-invalidLenByteCount",
			NewDecoder([]byte{UniversalObjectTag, 0x02}),
			nil,
			true,
		},
		{
			"-incorrect_tag",
			NewDecoder([]byte{0x01, 0x02, 0x01, 0x00}),
			nil,
			true,
		},
	}
